    ...
    def update_trip_by_id(self, new_info: TripSchema, trip_id: str):
        ...
        if trip_dict is not None:
            ...
            if new_info.departure_date and new_info.return_date:
                return self.check_departure_and_return_inputs(user_id, new_info.departure_date, new_info.return_date, trip_id,
                                                       new_info)
            # only departure_date was given
            elif new_info.departure_date and not new_info.return_date:
                return self.check_departure_and_return_inputs(user_id, new_info.departure_date, trip.return_date, trip_id,
                                                       new_info)
            # only return_date was given
            elif new_info.return_date and not new_info.departure_date:
                return self.check_departure_and_return_inputs(user_id, trip.departure_date, new_info.return_date, trip_id,
                                                       new_info)
